...
module.exports = {
   ...
   // 创建文章 管理员
   createArticle:function ({articleId,classType,title,content,contentToMark,createTime,brief}){
        return query(`INSERT INTO article(articleId,classType,title,content,contentToMark,createTime,brief) 
                    VALUES(${articleId},'${classType}','${title}','${content}','${contentToMark}','${createTime}','${brief}')`);
   },
   //获取文章列表 管理员
   ...
   //获取项目列表 管理员
   getProjsList: function(){
      return query(`SELECT projId,createTime,title FROM projs ORDER BY projId DESC`);
   },
   //查看项目列表 用户
   getProjsListNoAuth: function(){
      return query(`SELECT * FROM projs ORDER BY projId DESC`);
   },
   //创建项目 管理员
   createProj: function({createTime,title,descs,showSrc,codeSrc,blogSrc}){
      return query(`INSERT INTO projs(createTime,title,descs,showSrc,codeSrc,blogSrc) 
      VALUES('${createTime}','${title}','${descs}','${showSrc}','${codeSrc}','${blogSrc}')`);
   },
   //编辑项目 管理员
   updateProj: function({projId,title,descs,showSrc,codeSrc,blogSrc}){
    return query(`UPDATE projs SET 
    title = '${title}',
    descs = '${descs}',
    showSrc= '${showSrc}',
    codeSrc= '${codeSrc}',
   ...
   removeProj: function(projId){
    return query(`DELETE FROM projs WHERE projId = '${projId}'`);
   }
  }